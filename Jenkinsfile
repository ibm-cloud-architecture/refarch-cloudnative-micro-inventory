    ...
    node ('mypod') {
        container('gradle') {
            ...
            stage ('Build Docker Image') {
                sh 'cd catalog && ./gradlew docker && cd docker && docker build -t cloudnative/catalog-jenkins .'
            }
            stage ('Push Docker Image to Registry') {
                ...
                docker tag cloudnative/catalog-jenkins registry.ng.bluemix.net/chrisking/catalog:${env.BUILD_NUMBER}
                docker push registry.ng.bluemix.net/chrisking/catalog:${env.BUILD_NUMBER}
                """
            ...
            stage ('Deploy to Kubernetes') {
                ...
                #!/bin/bash
                cd catalog
                yaml w deployment.yml spec.template.spec.containers[0].image registry.ng.bluemix.net/chrisking/catalog-fabio-jenkins:${env.BUILD_NUMBER} > deployment2.yml
                kubectl --token=`cat /var/run/secrets/kubernetes.io/serviceaccount/token` create -f deployment2.yml
                kubectl --token=`cat /var/run/secrets/kubernetes.io/serviceaccount/token` create -f service.yml
                """*/
                //sh 'cd catalog && ./deploy.sh ${env.BUILD_NUMBER}'
                sh """
                #!/bin/bash
                cd catalog
